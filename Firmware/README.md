...
- **BTT Smart Filament Sensor V2.0** (runout switch + motion, catches jams): its motion signal is read on D9, next to the runout input D8 on the board's sensor plug. Off by default; wire it, then `M412 S1 L10` and `M500`. Wiring diagram and details: [Filament sensor](https://le-syl21.github.io/WANHAO-Duplicator-9/sensor.html). To build it, these release builds also turn on `FILAMENT_SWITCH_AND_MOTION` with `FIL_MOTION1_PIN 9` and a 10 km default jam length (`M412 S1` also turns the motion check on, and its count only restarts at power-on or `M600`), and load the default lengths instead of the zeros stored by v2.0.4 or earlier or the 100 m stored by v2.0.5 (one line in Marlin's `settings.cpp`).
...
- **The screen's *Information* page shows your model, size and release**, for example *Wanhao D9 MK2 300* and *2.1.x (v2.0.6)*. `M115` reports the same.
...
- **BTT Smart Filament Sensor V2.0** (fin de filament + mouvement, détecte les bourrages) : son signal de mouvement est lu sur D9, à côté de l'entrée de fin de filament D8 sur la prise capteur de la carte. Désactivé par défaut ; branchez-le, puis `M412 S1 L10` et `M500`. Schéma de branchement et détails : [Capteur filament](https://le-syl21.github.io/WANHAO-Duplicator-9/fr/sensor.html). Pour cela, ces firmwares activent aussi `FILAMENT_SWITCH_AND_MOTION` avec `FIL_MOTION1_PIN 9` et une longueur de bourrage de 10 km par défaut (`M412 S1` active aussi la surveillance du mouvement, dont le décompte ne repart qu'à l'allumage ou à un `M600`), et chargent les longueurs par défaut au lieu des zéros enregistrés par la v2.0.4 ou avant ou des 100 m enregistrés par la v2.0.5 (une ligne dans `settings.cpp` de Marlin).
...
- **La page *Information* de l'écran affiche votre modèle, votre taille et la version**, par exemple *Wanhao D9 MK2 300* et *2.1.x (v2.0.6)*. `M115` indique la même chose.
